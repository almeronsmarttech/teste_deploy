...
class Concreto:
    ...
    def __init__(self, fck=20, tipo_agregado: TipoAgregado = TipoAgregado.ARENITO, gama_c = 1.4, nu = 0.2):
        ...
        self.__fctk_m = self.calcular_fct_m()
    ...
    def calcular_fct_m(self):
        if self.__fck * 10 <= 50:
            #return 0.3 * ((self.fck * 10) ** (2 / 3)) / 10.
            return 0.3 * np.pow(self.__fck, 2 / 3) / 10  # em kN/cm2
        else:
            return 2.12 * np.log(1 + 0.11 * self.__fck) / 10.

    def calcular_eta_c(self):
        if self.__fck <= 40:
            return 1.
        else:
            return pow(40 / (self.fck), 1 / 3)

    def calcula_alfa_c(self):
        if self.__fck <= 50:
            return 0.85
        else:
